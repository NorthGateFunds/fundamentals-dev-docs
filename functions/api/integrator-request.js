...
// Cloudflare Pages Functions route for /api/integrator-request
//
// - Inserts integrator request via Supabase RPC
// - If Step 1 test (push_access + delivery_env=test), POSTs minimal NewsML payload
// - Returns delivery status + headers
//
// NOTE: Cloudflare signature is (context) => Response

export const onRequestPost = async (context) => {
  const HANDLER_VERSION = "integrator-request@2026-01-15T04:25Z";
  ...
  const buildPublicIdentifier = ({ providerId, dateId, newsItemId, revisionId }) =>
    `urn:newsml:${providerId}:${dateId}:${newsItemId}:${revisionId}`;

  const normalizeXhtmlDoc = (innerHtml, titleText) => {
  ...
  const makeTestNewsML = ({ providerId, providerName, createdIso, newsItemId }) => {
...
<NewsML xmlns="http://iptc.org/std/NewsML/1.2/" Version="1.2">
  ...
  const normalizeKindAndEnv = (kind, deliveryEnvRaw) => {
    const delivery_env = cleanStr(deliveryEnvRaw);

    if (kind === "push_test") return { dbKind: "push_access", delivery_env: delivery_env || "test", isTest: true };
    if (kind === "push_enable") return { dbKind: "push_access", delivery_env: delivery_env || "production", isTest: false };
    ...
    if (kind === "integration") return { dbKind: "integration", delivery_env, isTest: false };
  ...
  const pickHeaders = (headers) => {
    const out = {};
    const keys = ["content-type", "content-length", "server", "date", "cf-ray"];
    for (const k of keys) {
  ...
  try {
    ...
    return json(400, { ok: false, error: "invalid_json", v: HANDLER_VERSION });
  }

  const incomingKind = cleanStr(body.kind);
  const email = cleanStr(body.email)?.toLowerCase() ?? null;

  if (!incomingKind) return json(400, { ok: false, error: "invalid_kind", v: HANDLER_VERSION });
  if (!email || !isEmail(email)) return json(400, { ok: false, error: "invalid_email", v: HANDLER_VERSION });

  const { dbKind, delivery_env, isTest } = normalizeKindAndEnv(incomingKind, body.delivery_env || body.environment);
  ...
  if (dbKind === "push_access" && isTest) {
    if (!endpoint_url) return json(400, { ok: false, error: "missing_endpoint_url", v: HANDLER_VERSION });
    if (!isHttpsUrl(endpoint_url)) return json(400, { ok: false, error: "endpoint_must_be_https", v: HANDLER_VERSION });
  }
  ...
  const rpcPayload = {
    ...
    p_source_path: cleanStr(body.source_path) || context.request.headers.get("referer") || null,
  ...
  try {
    ...
    return json(502, { ok: false, error: "supabase_unreachable", v: HANDLER_VERSION });
  }
  ...
  if (dbKind === "push_access" && isTest) {
    ...
    return json(200, {
      ...
      response_snippet: String(respText || "").slice(0, 300),
      sent_bytes: xml.length,
      newsml_news_item_id: newsItemId,
      v: HANDLER_VERSION,
    });
  ...
  return json(200, { ok: true, id, v: HANDLER_VERSION });
};